...
- 로그인 성공 시 기본 `/studio`. OAuth 이메일 없음 시 `users.email`에 `{provider}_{authId}@oauth.novelagent.local` 형태로 저장.
- 미로그인 시 `/studio`, `/works/*`, **`/billing`**, **`/verify-phone`** → `/login` (middleware)
...
- **스튜디오** 등에 표시하는 숫자는 (탐색 UI 비활성 시에도 동일 계산 로직 사용) `reader_actions`가 아니라, 해당 작품에서 **AI 분석이 한 번이라도 저장된 회차**마다 최신 1건의 `overall_score`를 모은 **산술 평균**(반올림)입니다. 분석이 없으면 **—**.
