...
| 기능별 | `NAVER_CLIENT_ID`, `NAVER_CLIENT_SECRET` | 네이버 로그인. 없으면 `/api/auth/naver/start`가 오류 JSON 반환(로컬만 채우고 Vercel 비우면 흔한 실수) |
| 서버 전용(민감) | `SUPABASE_SERVICE_ROLE_KEY` | 회원 탈퇴 등 관리 API — **절대** `NEXT_PUBLIC_` 붙이지 말 것 |
