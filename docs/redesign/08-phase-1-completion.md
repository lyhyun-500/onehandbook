...
### Post (페이즈 1 종료 + production 배포 후, 2026-05-10)

> 측정 시점: 페이즈 1 묶음 push (Commit 1~6) → PR #2 main 머지 (`04f8fef`) → Vercel production 자동 배포 완료 → 시크릿 창 Lighthouse 측정.
>
> 측정 조건: Pre baseline 과 동일 — Mobile (Emulated Moto G Power), Slow 4G throttling, Initial page load, 시크릿 창.

| 항목 | Pre | Post | 변화 | 임계값 | 상태 |
|---|---|---|---|---|---|
| **Performance** | 80 🟡 | **97 🟢** | **+17점** | 90+ | 🟢 |
| FCP | 2.9s 🔴 | **0.9s 🟢** | -2.0s (-69%) | < 1.8s | 🟢 |
| LCP | 3.9s 🔴 | 2.6s 🔴 | -1.3s (-33%) | < 2.5s | 🔴 (0.1s 차) |
| TBT | 190ms 🟢 | **60ms 🟢** | -130ms (-68%) | < 200ms | 🟢 |
| CLS | 0 🟢 | 0.001 🟢 | +0.001 | < 0.1 | 🟢 |
| Speed Index | 2.9s 🟢 | **0.9s 🟢** | -2.0s (-69%) | < 3.4s | 🟢 |
| Render-blocking | 700ms 🔴 | **80ms 🟢** | -620ms (-89%) | — | 🟢 |
...
1. **단일 root cause 제거가 +17점 회수** — Commit 1 (`Noto_Sans_KR` `subsets:["latin"]` 모순 제거, 308KB CSS 절감) 의 production 효과 검증. 정확한 진단의 ROI.
2. **FCP 69% 감소 (2.9s → 0.9s)** — 사용자가 화면 박힌 걸 보는 시점 2초 빨라짐. render-blocking 절감 (700ms → 80ms, -89%) 의 직접 효과.
3. **LCP 임계값 미통과 (2.6s, 목표 < 2.5s, 0.1s 차)** — 페이즈 5 종료 시점 통과 가능성:
   - "Improve image delivery" 13 KiB 절감 가능
   - "Reduce unused JavaScript" 150 KiB 절감 가능
   - 페이즈 2~5 페이지 마이그레이션 시 next/image + `fetchpriority="high"` 적용
4. **CLS 0 → 0.001 미세 증가** — 무시 가능 수준이나 페이즈 2 진입 시 LCP 이미지 처리 박을 때 함께 살펴봄.
5. **TBT 68% 감소 (190ms → 60ms)** — 토큰/atoms 도입이 JS 비용 거의 없음 + render-blocking 절감으로 main thread 여유 확보.

### 페이즈 5 종료 후 비교 목표 (참고)

| 항목 | Pre | Post (페이즈 1 끝) | 페이즈 5 목표 |
|---|---|---|---|
| Performance | 80 | **97** | 95+ 유지 |
| LCP | 3.9s | 2.6s | **< 2.5s (Core Web Vitals 통과)** |
| FCP | 2.9s | 0.9s | 유지 (< 1.8s) |
| TBT | 190ms | 60ms | 유지 (< 200ms) |
| CLS | 0 | 0.001 | 유지 |
