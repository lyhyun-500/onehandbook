...
const WEBHOOK_ENV = "production" as const;
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    console.error("[paddle webhook production] 요청 본문 읽기 실패:", e);
  ...
  try {
    ...
    console.error("[paddle webhook production] Paddle 서버 설정 로드 실패:", e);
  ...
  if (!verification.valid) {
    console.error(
      "[paddle webhook production] 서명 검증 실패:",
  ...
  try {
    ...
    console.error("[paddle webhook production] JSON 파싱 실패:", e);
  ...
  if (!notificationId) {
    ...
    console.error(
      "[paddle webhook production] notification_id 없음 — payload 스키마 확인 필요"
    );
    return NextResponse.json({ ok: false, skipped: true }, { status: 200 });
  }

  const eventType = extractEventType(event);
  console.log("[paddle webhook production] 수신:", {
  ...
  try {
    ...
    if (selectError) {
      console.error("[paddle webhook production] paddle_webhooks 조회 실패:", selectError);
    ...
    if (upsertError) {
      console.error("[paddle webhook production] paddle_webhooks upsert 실패:", upsertError);
      return NextResponse.json({ error: "db_error" }, { status: 500 });
    }

    if (!isPaddleWebhookEvent(event)) {
      console.error("[paddle webhook production] 알 수 없는 webhook 이벤트 형식:", {
        notification_id: notificationId,
        event_type: eventType,
      });
    ...
    waitUntil(
      processPaddleWebhookEvent(event).catch((err) => {
        console.error("[paddle webhook production] 백그라운드 처리 실패:", err);
    ...
    console.error("[paddle webhook production] 처리 중 예외:", e);
